...
from centaurohybridmpc.utils.jnt_imp_cntrl import OmniJntImpCntrl

class CentauroHybridStepping(BaseTask):
    def __init__(self, 
                name: str, 
                num_envs = 1,
                device = "cuda", 
    ...
    def _generate_srdf(self,
        ...
        # we generate the URDF where the Centauro description package is located
        import rospkg
        rospackage = rospkg.RosPack()
        descr_path = rospackage.get_path("centauro_srdf")
        ...
        add_legs = "legs:=" + legs
        use_big_wheels = "big_wheel:=" + big_wheel
        use_upper_body = "upper_body:=" + upper_body
        add_velodyne = "velodyne:=" + velodyne
        add_realsense = "realsense:=" + realsense
        ...
        try:
            ...
            raise Exception(f"[{self.__class__.__name__}]" + f"[{self.exception}]" + ": failed to generate SRDF!!!")
    ...
    def _generate_urdf(self, 
        ...
        import rospkg
        rospackage = rospkg.RosPack()
        descr_path = rospackage.get_path("centauro_urdf")
        ...
        try:
            ...
            raise Exception(f"[{self.__class__.__name__}]" + f"[{self.exception}]" + ": failed to generate URDF!!!")
